...
public class RestoreProcessor implements PeriodicTask
{
    ...
    private final Set<RestoreRangeHandler> activeTasks = ConcurrentHashMap.newKeySet();
    ...
    {
        for (RestoreRangeHandler task : activeTasks)
        {
            ...
            if (elapsedInSeconds > longRunningHandlerThresholdInSeconds)
            {
                ...
                task.range().owner().metrics().restore().slowRestoreTaskTime.metric.update(elapsedInNanos, TimeUnit.NANOSECONDS);
            }
        }
    }
